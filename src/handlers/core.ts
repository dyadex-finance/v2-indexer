  ...
  async ({ event, context }) => {
    try {
      ...
      if (from === ADDRESS_ZERO) {
        ...
        const existingMints = await context.Mint.getWhere({
      ...
      if (to === pair.id) {
        ...
        const existingBurns = await context.Burn.getWhere({
          transaction_id: { _eq: transactionId },
        });
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    try {
      ...
      const updatedToken0: Token = {
        ...token0,
        txCount: token0.txCount + ONE_BI,
      };

      const updatedToken1: Token = {
        ...token1,
      ...
      const updatedFactory: UniswapFactory = {
        ...factory,
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    try {
      ...
      const bundle = await context.Bundle.get(`${chainId}-1`);
      let finalToken0: Token | undefined;
      let finalToken1: Token | undefined;
      ...
      // 12. Update daily/hourly data
      if (bundle) {
        const pairDayData = await updatePairDayData(
          pair,
          event,
          context,
          String(chainId),
        );
        const pairHourData = await updatePairHourData(
          pair,
          event,
          context,
          String(chainId),
        );
        const uniswapDayData = await updateUniswapDayData(
          event,
          context,
          String(chainId),
        );
        ...
        if (uniswapDayData) {
          const updatedUniswapDayData = {
            ...uniswapDayData,
            dailyVolumeUSD:
              uniswapDayData.dailyVolumeUSD.plus(trackedAmountUSD),
            dailyVolumeETH:
              uniswapDayData.dailyVolumeETH.plus(trackedAmountETH),
            dailyVolumeUntracked:
              uniswapDayData.dailyVolumeUntracked.plus(derivedAmountUSD),
            // Update total volume fields (matches subgraph exactly)
            totalVolumeUSD:
              uniswapDayData.totalVolumeUSD.plus(trackedAmountUSD),
            totalVolumeETH:
              uniswapDayData.totalVolumeETH.plus(trackedAmountETH),
          };
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    try {
      ...
      const updatedPairWithPrices: Pair = {
        ...pair,
        reserve0: reserve0,
        reserve1: reserve1,
      ...
      if (bundle) {
        ...
        const token0DerivedETH = await findEthPerToken(
          updatedToken0,
          context,
          chainId,
        );
        const token1DerivedETH = await findEthPerToken(
          updatedToken1,
          context,
          chainId,
        );
        ...
        const finalToken0: Token = {
          ...updatedToken0,
          derivedETH: token0DerivedETH,
        };

        const finalToken1: Token = {
          ...updatedToken1,
        ...
        const finalFactory: UniswapFactory = {
          ...updatedFactory,
          totalLiquidityETH:
        ...
        const finalToken0WithLiquidity: Token = {
          ...finalToken0,
          totalLiquidity: finalToken0.totalLiquidity.plus(reserve0),
        };

        const finalToken1WithLiquidity: Token = {
          ...finalToken1,
